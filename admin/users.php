...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'edit') {
        ...
        try {
            ...
            $msg = 'อัปเดตข้อมูลผู้ใช้งาน สำเร็จ';
            $msg_type = 'success';
        } catch (PDOException $e) {
            $msg_type = 'danger';
            $msg = 'เกิดข้อผิดพลาดในการอัปเดต: ' . $e->getMessage();
        }
    }
...
<body>
    ...
    <main class="main-content">
                        ...
                        <tr>
                            ...
                            <td style="display: flex; gap: 10px;">
                                <button type="button" 
                                    ...
                                    data-id="<?= $u['id'] ?>"
                                    data-fname="<?= htmlspecialchars($u['firstname']) ?>"
                                    data-lname="<?= htmlspecialchars($u['lastname']) ?>"
                                    data-user="<?= htmlspecialchars($u['username']) ?>"
                                    data-pass="<?= htmlspecialchars($u['password']) ?>"
                                    data-role="<?= htmlspecialchars($u['role']) ?>"
                                    data-aff="<?= htmlspecialchars($u['Affiliation'] ?? '') ?>"
                                    data-affname="<?= htmlspecialchars($u['affiliation_item'] ?? '') ?>"
        ...
        <!-- EDIT USER MODAL -->
        <div id="edit-user-modal" style="display:none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 1000; justify-content: center; align-items: center;">
            <div class="card" style="width: 100%; max-width: 650px; position: relative; max-height: 90vh; overflow-y: auto;">
                <button type="button" onclick="document.getElementById('edit-user-modal').style.display='none'" style="position: absolute; top: 20px; right: 20px; background:none; border:none; font-size:1.5rem; cursor:pointer;">&times;</button>
                <div class="card-title" style="margin-bottom: 1.5rem;">แก้ไขข้อมูลผู้ใช้งาน</div>
                <form method="POST" action="users.php">
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            <label class="form-label">ชื่อ</label>
                            <input type="text" class="form-control" name="firstname" id="e_fname" required>
                        </div>
                        <div class="form-group">
                            <label class="form-label">นามสกุล</label>
                            <input type="text" class="form-control" name="lastname" id="e_lname" required>
                        </div>
                        <div class="form-group">
                            <label class="form-label">Username</label>
                            <input type="text" class="form-control" name="username" id="e_user" required>
                        </div>
                        <div class="form-group">
                            <label class="form-label">Password</label>
                            <input type="text" class="form-control" name="password" id="e_pass" required>
                        </div>
                        <div class="form-group">
                            <label class="form-label">Email</label>
                        ...
                        <div class="form-group">
                            <label class="form-label">ตำแหน่ง</label>
                            <input type="text" class="form-control" name="position" id="e_pos">
                        </div>
                        <div class="form-group">
                            <label class="form-label">สิทธิ์</label>
                            <div class="custom-select-wrapper" id="e-role-select">
                                <input type="hidden" name="role" id="e_role" required>
                                <div class="custom-select-trigger" onclick="toggleSelect('e-role-select')">
                                    <span id="e-role-label">เลือกสิทธิ์</span>
                                    <svg class="custom-select-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 12 15 18 9"></polyline></svg>
                                </div>
                                <div class="custom-options">
                        ...
                        <div class="form-group">
                            <label class="form-label">สังกัดงาน</label>
                            <div class="custom-select-wrapper" id="e-affiliation-select">
                                <input type="hidden" name="affiliation" id="e_aff" required>
                                <div class="custom-select-trigger" onclick="toggleSelect('e-affiliation-select')">
                                    <span id="e-affiliation-label">เลือกสังกัดงาน</span>
                                    <svg class="custom-select-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 12 15 18 9"></polyline></svg>
                                </div>
                                <div class="custom-options">
                                    <?php foreach ($affiliations as $a): ?>
                                        <div class="custom-option" onclick="selectOption('e-affiliation-select', '<?= $a['id'] ?>', '<?= htmlspecialchars($a['affiliation_item']) ?>')">
                                            <?= htmlspecialchars($a['affiliation_item']) ?>
                                        </div>
                                    <?php endforeach; ?>
                                </div>
                    ...
                    <div class="form-actions" style="margin-top: 1.5rem;">
                        <button type="button" class="btn btn-cancel" onclick="document.getElementById('edit-user-modal').style.display='none'">ยกเลิก</button>
    ...
    <script>
        ...
        function openEditModal(btn) {
            try {
                ...
                document.getElementById('e_fname').value = btn.getAttribute('data-fname');
                document.getElementById('e_lname').value = btn.getAttribute('data-lname');
                document.getElementById('e_user').value = btn.getAttribute('data-user');
                document.getElementById('e_pass').value = btn.getAttribute('data-pass');
                document.getElementById('e_email').value = btn.getAttribute('data-email');
                document.getElementById('e_pos').value = btn.getAttribute('data-pos');
                
                // Set custom selects (UI sync)
                const role = btn.getAttribute('data-role');
                selectOption('e-role-select', role, role);

                const affId = btn.getAttribute('data-aff');
                const affName = btn.getAttribute('data-affname');
                if (affId && affId !== '') {
                    selectOption('e-affiliation-select', affId, (affName && affName !== '') ? affName : 'เลือกสังกัดงาน');
                } else {
                    selectOption('e-affiliation-select', '', 'เลือกสังกัดงาน');
                }
